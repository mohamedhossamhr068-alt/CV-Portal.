...
ui = {
    "العربية": {
        ...
        "login_btn": "تسجيل الدخول", "signup_btn": "إنشاء حساب جديد",
        "logout": "خروج 🚪",
        ...
        "ai_prompt": "أنت خبير موارد بشرية. أعد صياغة البيانات التالية لسيرة ذاتية احترافية ATS بالعربية.\nالاسم: {name}\nالمسمى: {job_title}\nالخبرات: {experience}\nالمهارات: {skills}\n\nرتّب: الملخص المهني، الخبرات، المهارات. استخدم نقاط وأفعال قوية.",
    },
    "English": {
        ...
        "quota_error": "⛔ Free limit reached.",
        ...
        "no_jobs": "No matching jobs found.",
        "linkedin_btn": "💼 View Jobs on LinkedIn",
        "admin_title": "⚙️ Admin Dashboard",
        "ai_prompt": "You are an elite HR Expert. Rewrite into an ATS-optimised résumé.\nName: {name}\nJob Title: {job_title}\nExperience: {experience}\nSkills: {skills}\n\nSections: Professional Summary, Work Experience, Skills. Use bullet points and action verbs.",
    },
}[lang]
...
if st.session_state.user is None:
    ...
    with col_m:
        ...
        <div class="section-card" style="margin-top:60px; text-align:center;">
            ...
            <h1 style="font-size:1.6rem;">{ui["login_title"]}</h1>
            <p style="color:#64748B;">{ui["login_sub"]}</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    <div class="hero-banner">
        <div style="display:flex; justify-content:space-between; flex-wrap:wrap; gap:12px;">
            ...
            <div style="min-width:180px;">
                <div style="font-size:13px; color:#BAE6FD;">{ui["usage"].format(count=usage_count)}</div>
    ...
    if submitted and name and job_title:
        ...
        with col1:
            ...
            with st.spinner(ui["loading_ai"]):
                prompt = ui["ai_prompt"].format(name=name, job_title=job_title, experience=experience, skills=skills)
                try:
                    resp = requests.post(
                    ...
                    result = resp.json()
                    if "error" in result:
                        st.error(f"AI error: {result['error']['message']}")
                    else:
                        ai_text = result["choices"][0]["message"]["content"]
                        st.markdown(f'<div class="resume-output">{ai_text.replace(chr(10), "<br>")}</div>', unsafe_allow_html=True)
        ...
        with col2:
            ...
            with st.spinner(ui["loading_jobs"]):
                ...
                st.markdown(f'<a class="linkedin-btn" href="https://www.linkedin.com/jobs/search/?keywords={li_q}&location={li_loc}" target="_blank">{ui["linkedin_btn"]}</a>', unsafe_allow_html=True)
                st.markdown(f"<p style='font-weight:600;'>{ui['wuzzuf_header']}</p>", unsafe_allow_html=True)
                try:
                    q = urllib.parse.quote(f"{job_title} {location}")
                    wres = requests.get(f"https://wuzzuf.net/search/jobs/?q={q}&a=hpb", headers={"User-Agent": "Mozilla/5.0"}, timeout=15)
                    ...
                    if not cards:
                        ...
                        for card in cards:
                            ...
                            if title_el and company_el:
                                ...
                                st.markdown(f'<div class="job-card"><a href="{job_link}" target="_blank" style="color:#1E3A8A; font-weight:700; text-decoration:none;">{title_el.text.strip()}</a><div style="color:#64748B; font-size:13px; margin-top:6px;">🏢 {company_el.text.strip().replace(" -", "")}</div></div>', unsafe_allow_html=True)
                except Exception:
